...
class LedgerWallet {
  ...
  constructor(path) {
    ...
    if (!allowed_hd_paths.some(hd_pref => path.startsWith(hd_pref))) { throw new Error(`hd derivation path for Nano Ledger S may only start [${allowed_hd_paths}], ${path} was provided`); }
    this._path = path;
    this._accounts = null;
  ...
  async signTransaction(txData, callback) {
    ...
    const tx = new EthereumTx(txData);
        ...
        // Fetch the chain id
    web3.version.getNetwork(async (error, chain_id) => {
